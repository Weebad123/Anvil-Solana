...
fn reserve_collateral_internal<'info>(ctx: &mut Context<'_, '_, 'info, 'info, ReserveCollateral<'info>>,
    ...
    Result<u64> {
        ...
        let available_amount = ctx.accounts.account_balance_pda.collateral_balance.available;
        ...
        ctx.accounts.account_balance_pda.collateral_balance.available = 
            ctx.accounts.account_balance_pda.collateral_balance.available - reserved_collateral;
        ctx.accounts.account_balance_pda.collateral_balance.reserved = 
            ctx.accounts.account_balance_pda.collateral_balance.reserved + reserved_collateral;
